  ...
  users: 8, // количество маркеров
	...
	max_guests: 10,
	time in: ['12:00', '13:00', '14:00'],  // checkin, checkout
	time out: ['12:00', '13:00', '14:00'],
...
/* "avatar": строка, адрес изображения вида img/avatars/user{{xx}}.png,
	...
	*/
	...
	userLocation.style.left = (location.x - all.marker_height) + 'px';
	userLocation.style.top = location.y - (all.marker_width / 2) + 'px';
...
mapPin.appendChild(createPin(MapPin2));
	...
	for (var i = 0; i < all.users; i++) {
	 var location.x = getRandomNumber(all.min_x, all.max_x);
	 var location.y = getRandomNumber(all.min_y, all.max_y);
	      }
...
add_all {
	 ...
	 offer: {
	   ...
	   features: getArrayLenght(all.service),                             // features - массив строк случайной длины из ниже предложенных
	   ...
	   guests: getRandomNumber(all.min_guests, all.max_guests),           // "guests": число, случайное количество гостей, которое можно разместить
	   description: '',
...
var adsList = windowText	
  adsList.querySelector('.popup__title').textContent = offer.title;
	adsList.querySelector('.popup__address').textContent = offer.adress;
	adsList.querySelector('.popup__price').innerHTML = getPrice(offer.price);
	adsList.querySelector('.popup__type').textContent = rusType(offer.type);
	adsList.querySelector('.popup__rooms-and-guests').textContent = getGuestsRooms(offer.guests, offer.rooms);
	adsList.querySelector('.popup__checkin-time').textContent = getTime(offer.checkin, offer.checkout);
	adsList.querySelector('.popup__features').appendChild = offer.features;
	adsList.querySelector('.popup__description').textContent = offer.description;
	adsList.querySelector('.popup__photos').appendChild = offer.photos;
	...
	var newFeatures = offer.service.slice();
	 newFeatures.length = getRandomNumber(offer.service.length);
	 return newFeatures;
	}
